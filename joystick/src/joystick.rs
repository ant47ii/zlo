...
impl<R: AsyncAdcReader> Joystick<R> {
	...
	fn process_axis(&self, raw: u16, zero: u16) -> i8 {
		...
		if delta.abs() < 5 { return 0; }
		let max_range = if delta > 0 { 255 - zero as i32 } else { zero as i32 };
		if max_range == 0 { return 0; }
		((delta * 100) / max_range).clamp(-100, 100) as i8
	}
}

pub fn apply_joystick_expo(raw_input: i8, expo_factor: f32) -> i8 {
